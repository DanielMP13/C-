...
class Program
{
    ...
    static void ConsultarProducto()
    {
        using (SqlConnection conexion = new SqlConnection(CadenaConexion))
        {
            ...
            using (SqlDataReader lector = comando.ExecuteReader())
            {
                ...
                while (lector.Read())
                {
                    Console.WriteLine($"ID: {lector["Id"]}, Nombre: {lector["Nombre"]}, Cantidad: {lector["Cantidad"]}");
                }
            }
        }
    }
    static void InsertarProducto()
    {
        ...
        {
            conexion.Open();
            string consulta = "INSERT INTO Productos (Nombre, Cantidad) VALUES (@nombre, @cantidad)";
        ...
        while (seguir.ToUpper() == "S")
        {
                ...
                else
                {
                    ...
                    using (SqlConnection conexion = new SqlConnection(CadenaConexion))
                    {
                        ...
                        // Verificar si hay suficiente stock antes de proceder con la venta
                        string consultaStock = "SELECT Cantidad FROM Productos WHERE Id = @id";
                        using (SqlCommand comandoStock = new SqlCommand(consultaStock, conexion))
                        {
                            ...
                            using (SqlDataReader lector = comandoStock.ExecuteReader())
                            {
                                if (lector.Read())
                                {
                                    ...
                                    {
                                        Console.WriteLine($"⚠ No hay suficiente stock. Stock actual: {cantidadActual}");
                                        continue;
                                    }
                                }
                                else
                        ...
                        // Actualizar la cantidad del producto después de la venta
                        ...
                        string consultaRegistroVenta = "INSERT INTO Ventas (IdProducto, Cantidad,FechaVenta) VALUES (@idProducto,@cantidad, @fecha)";
                        ...
                        Console.WriteLine($"✔ Se registró la venta de {cantidadAVender} unidad(es) del producto con ID {id}.");
                    }
                }
